...
| [71-识图不再阻塞回复](71-识图不再阻塞回复-20260913.md) | `dsh-imgctx` 1.2.0：把「节奏不自然」归因到具体代码——按「这一轮有没有夹识图」给被@回复分组，8.6s vs 25.8s；改成只有当前消息自带的图和用户精确引用的图才阻塞，历史顺带的图只吃缓存、其余后台预转述（下一轮白拿）；含 40% 预算放弃率这一信号、env 值背离代码注释设计意图（`BUDGET=24` vs 注释「不能半分钟不说话」）、`pkill -f` 自匹配、以及**修正** docs/68 那条「容器内跑测试不污染生产日志」的核对方式 |
