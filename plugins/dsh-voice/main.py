...
class Main(star.Star):
    ...
    async def send_voice(self, event: AstrMessageEvent, text: str):
        ...
        if not ok:
            ...
            # [patch:no-canned-fail] 这里**不能指定模型说什么**。
            # 旧文案写的是「告诉用户这一句没念出来、让他换个说法或稍后再说」，
            # 模型于是逐字照搬 —— 2026-09-15 群里两次出现固定句
            # 「这句先没念出来，换个说法或者稍后再试」，同一位群友两次把它引用
            # 回来问「为什么😢」：既像机器人（真人不会汇报自己发没发出去），又把
            # 责任推给说话的人。err 原文也不再进 prompt（以前出现过模型照抄
            # 「发不出声音：审核通道超时」把内部机制播到群里）。
            # 真人的做法是：语音没成，就把那句话用文字说了，不汇报、不甩锅。
            if err == BANNED_HINT:
                return (
                    "这段内容不适合念，语音已放弃，这段内容也不要发出来。"
                    "不要提语音、审核、关键词这类词，也不要让用户换个说法；"
                    "用普通文字自然回一句别的话就行。"
                )
            return (
                "语音这次没成，改发文字。不要提语音、审核、超时、通道这类词，"
                "也不要让用户换个说法或重试 —— 直接把原本想说的那句话用普通文字"
                "短句发出来，就像一开始就没打算发语音。"
            )
        return "语音已经发出去了。请只回一句很短的话，不要重复语音里的内容。"
