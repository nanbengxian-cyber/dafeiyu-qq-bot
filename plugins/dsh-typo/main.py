...
补纠正默认 8%（DSH_TYPO_FIX_RATE）。这个数是照着群友的真实频率倒推的：
主群群友每千条消息出现「打错了/打错字/手滑」约 0.87 次，机器人原本
（0.35）是 3.65‰，高出 4 倍；0.08 折算下来约 0.83‰，和群友持平。
原来那个 35% 等于把「会犯错的真人」演成了「每次犯错都要报备的机器」——
群里 09-06 就有人直说过「不用说打错了我们看得懂的」。
...
# 打错之后补一条「真，打错了」的概率（在已经打错的前提下）。
# 8% 是照群友频率倒推的：0.06 × 0.08 ≈ 0.5% 的回复会带纠正，落在群友
# 0.87‰ 的同一量级。调高就等于把「会犯错真人」演成「犯错必报备的机器」。
FIX_RATE = min(1.0, max(0.0, float(os.environ.get("DSH_TYPO_FIX_RATE", "0.08"))))
...
def correction_for(text: str, i: int, form: float = 0.0) -> str:
    ...
    2026-09-13 去掉「*字」这一式。原来注释写「真人本来就常常只补一个字：
    『*的』『*很』」—— 那是没量过的假设。实测主群 27732 条群友消息，
    「*」开头的只有 1 条，而且是 `***`（屏蔽脏话），不是纠正。
    群友根本不用这个记法，机器人却用了 36 次（3.55‰ vs 0.04‰，98 倍），
    是纯打字习惯上的破绽。
    """
    right = text[i]
    forms = ("%s，打错了" % right, "打错，%s" % right)
