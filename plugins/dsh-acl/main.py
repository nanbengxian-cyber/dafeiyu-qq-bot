...
OWNER_CMDS = {
    ...
    # —— 会触发一次 LLM 调用的测试指令。
    "欢迎测试",
    # —— 以下大多是「暴露全群策略/运维参数」或「改全群共享状态」，
    #    插件内部自己也按 QQ 号判；这里登记进目录，让 /权限 能完整列出，
    #    并对没资格的人也在门口拦一层。
    "主动开口状态",
    "主动开口测试",
    "情绪状态",
    ...
    "入群审核模式",
    "入群问题",
    # —— 赞助收款（dsh-pay）。/赞助模式 切换 shadow→live 是整群行为变化，
    #    /赞助重置 动的是公共防刷屏闸，两者都只有群主能动。
    "赞助模式",
    "赞助重置",
    # —— dsh-aiflavour 动态AI味拦截。改模式/看名单/清名单 = 整群出口把关。
    "AI味状态",
    "AI味模式",
    "AI味名单",
    "AI味清名单",
    # —— dsh-armor 防破甲。改模式/看名单/清名单 = 群安全策略，只有群主能动。
    "破甲状态",
    "破甲模式",
    "破甲名单",
    "破甲清名单",
    # —— dsh-interest 今日馋/热度榜：整群情绪档案，只有群主能看。
    "馋什么",
    # —— dsh-proactive 探头状态：今日开口次数/冷却计算，预览群运营数据。
    "探头状态",
    # —— 2026-09-10 补登记：以下是后加的插件，之前只靠插件内部判身份，
    #    门卫表里没有 → /权限 列不出来（用户报「权限指令不是最新的」就是这里）。
    #    登记进 owner 档：/权限 能完整列出，门口也多一层拦。
    #    dsh-social 社交关系：整群亲疏口吻策略与手工档位。
    "关系状态",
    "关系标签",
...
ADMIN_CMDS = {
    ...
    "欢迎状态",
    "聊天记录状态",
    # 这两个插件内部没判身份，之前一直没人管、谁都能发。
    # 登记进管理员档：/拟人状态 /脊梁状态 都是运维数字，和上面同类。
    "拟人状态",
    "脊梁状态",
    # token 用量 = 花了多少钱。
    "stats",
    # —— 诊断类新指令。
    "赞助状态",   # 赞助收款当前模式/统计，只读
    "权限状态",   # dsh-acl 自身的权限目录，只有管理员能看是谁在管什么
    # —— dsh-noise 噪点注入统计（换单字/截断/叠词次数），是诊断数字。
    "噪点状态",
    # —— dsh-listen 听语音识别配置与战绩（成功/失败/平均耗时），诊断类。
    "听语音状态",
    # —— dsh-steal 表情包库存明细（总数/计数分布），只读运维信息。
    "表情库",
}
...
PUBLIC_CMDS = {
    ...
    "sid",
    "权限",
    # dsh-steal 偷来的表情包：随机发一张是群友娱乐，人人可用。
    "表情包",
    # —— dsh-social 自助指令（2026-09-10 补登记）：看自己的档位 / 删自己的
    #    社交关系记录，作用域只有自己，人人可用。之前没登记 → /权限 不显示。
    "我和鱼",
    "社交退出",
}
