...
import { safeFetchBuffer, validateFetchUrl, looksLikeImageBuffer, trustFetchHostFromUrl } from './safe-fetch.js';
...
const STICKER_PACK_FILE = path.join(STATE_DIR, 'sticker-packs.json');
...
function loadConfig() {
  ...
  const cfg = {
    ...
    socialV2: {
      ...
      stickerPack: {
        enabled: true,             // 本地表情包总开关
        dir: path.join(ROOT, '..', '表情包集1'), // 本地表情包目录（config.json 可覆盖）
        maxBytes: 8 * 1024 * 1024,  // 单张本地表情文件大小上限
        includeInPrompt: true,      // 是否在 qq_get_prompt / 唤醒提示里附带本地表情摘要与策略
        promptMaxPacks: 12,         // 提示里最多列出的本地表情包数
        allowedExt: ['.gif', '.png', '.jpg', '.jpeg', '.webp'], // 允许的表情文件扩展名
        cadence: {
          enabled: true,            // 发表情节奏开关：每连发 N 句话按概率提醒发一张
          everyMessages: 3,         // 每连续自发多少句文本后评估一次
          probability: 0.5          // 到边界时发一次表情的概率（0~1）
        }
      },
      proactive: {
        enabled: true,
  ...
  cfg.socialV2.tools = {
    ...
    collectSticker: true,
    getSelfImage: true,
    listLocalStickers: true,
    getLocalStickerImage: true,
    sendLocalSticker: true,
    ...(cfg.socialV2.tools ?? {})
  };
  ...
  // socialV2.stickerPack 同样需要深度合并，避免旧配置缺失子项时丢默认值。
  cfg.socialV2.stickerPack = {
    enabled: true,
    dir: path.join(ROOT, '..', '表情包集1'),
    ...
    cadence: {
      ...
      everyMessages: 3,
      probability: 0.5,
      ...((cfg.socialV2?.stickerPack?.cadence) ?? {})
    },
    ...(cfg.socialV2?.stickerPack ?? {})
  };
...
async function main() {
  ...
  // OneBot 后端（SnowLuma/NapCat 等）常把图片托管在自己的内网 HTTP 服务上，
  // 默认 SSRF 防护会把这些私网 URL 全部拦掉，导致 AI 永远读不到图。
  // 这里把「配置里显式写明的后端主机」加入受信名单，其余内网地址仍全部拦截。
  {
    const trustedFetchHosts = [];
    for (const rawUrl of [cfg.snowluma?.httpUrl, cfg.snowluma?.wsUrl]) {
      const host = trustFetchHostFromUrl(rawUrl);
      if (host && !trustedFetchHosts.includes(host)) trustedFetchHosts.push(host);
    }
    if (trustedFetchHosts.length) {
      log(`图片抓取受信主机（OneBot 后端）：${trustedFetchHosts.join(', ')}`);
    }
  }
  ...
  // ── 本地表情包（表情包集目录下的分类 GIF） ──────────────────────────────────
  let localPackEntries = loadLocalPacks(STICKER_PACK_FILE); // 内存态（含使用统计）
  let localPackScannedAt = 0; // 上次扫描目录的时间戳

  function localStickerPackEnabled() {
    return cfg.socialV2?.stickerPack?.enabled !== false;
  }

  // 解析本地表情包目录；返回绝对路径，目录不存在/非法时返回 null。
  function localPackDirPath() {
    const raw = String(cfg.socialV2?.stickerPack?.dir || '').trim();
    const dir = raw ? path.resolve(raw) : path.join(ROOT, '..', '表情包集1');
    if (!dir) return null;
    return dir;
  }

  function localPackAllowedExt() {
    const arr = Array.isArray(cfg.socialV2?.stickerPack?.allowedExt)
      ? cfg.socialV2.stickerPack.allowedExt.map((e) => String(e).toLowerCase())
      : ['.gif', '.png', '.jpg', '.jpeg', '.webp'];
    return arr.length ? arr : ['.gif'];
  }

  // 扫描本地表情包目录并合并持久化使用统计；force=true 时忽略缓存强制重扫。
  // 返回 { entries, dir, scannedAt, fromCache }；目录不可用（禁用/不存在）返回 null。
  function syncLocalPackLibrary(force = false) {
    if (!localStickerPackEnabled()) return null;
    const dir = localPackDirPath();
    if (!dir) return null;
    const now = Date.now();
    if (!force && localPackScannedAt && now - localPackScannedAt < 15000) {
      return { entries: localPackEntries, dir, scannedAt: localPackScannedAt, fromCache: true };
    }
    const scanned = scanLocalStickerDir(dir);
    localPackEntries = mergeLocalPacks(localPackEntries, scanned);
    localPackScannedAt = now;
    if (scanned.length) saveLocalPacks(STICKER_PACK_FILE, localPackEntries);
    log(`[sticker-pack] 已扫描本地表情包目录：${scanned.length} 个分类（${dir}）`);
    return { entries: localPackEntries, dir, scannedAt: now, fromCache: false };
  }

  // 取某个本地表情的条目；本地找不到时强制重扫一次（目录可能在会话中新增）。
  async function getLocalStickerData(ref) {
    let synced = syncLocalPackLibrary(false);
    let entry = findLocalSticker(synced?.entries ?? localPackEntries, ref);
    if (!entry) {
      synced = syncLocalPackLibrary(true);
      entry = findLocalSticker(synced?.entries ?? localPackEntries, ref);
      if (!entry) throw new Error(`找不到本地表情 ${ref}，请先用 qq_list_local_stickers 获取有效 id`);
    }
    return entry;
  }

  // 发送一个本地表情包（读文件 → base64 → image 段，发到群/私聊）。
  // 跨机拓扑：OneBot 在 PC，读不到手机上的本地绝对路径，因此用 base64 直传（不依赖 URL/本地路径）。
  async function sendLocalStickerV2(key, ref, options = {}) {
    const entry = await getLocalStickerData(ref);
    if (!entry.file || !fs.existsSync(entry.file)) throw new Error(`本地表情 ${entry.id} 的文件不存在，请检查表情包目录配置`);
    if (!fs.statSync(entry.file).isFile()) throw new Error(`本地表情 ${entry.id} 不是文件`);
    const ext = path.extname(entry.file).toLowerCase();
    if (!localPackAllowedExt().includes(ext)) throw new Error(`本地表情 ${entry.id} 的文件类型 ${ext} 不在允许列表内`);
    const maxBytes = Math.min(16 * 1024 * 1024, Math.max(0, Number(cfg.socialV2?.stickerPack?.maxBytes) || 8 * 1024 * 1024));
    const stat = fs.statSync(entry.file);
    if (stat.size > maxBytes) throw new Error(`本地表情 ${entry.id} 文件过大（${Math.round(stat.size / 1024)}KB > ${Math.round(maxBytes / 1024)}KB），已拒绝发送`);
    const buf = fs.readFileSync(entry.file);
    if (!looksLikeImageBuffer(buf)) throw new Error(`本地表情 ${entry.id} 不是有效的图片文件`);
    ...
    const updated = markLocalPackUsed(localPackEntries, entry.id, 'sticker-pack');
    localPackEntries = updated.entries;
    saveLocalPacks(STICKER_PACK_FILE, localPackEntries);
  ...
  // ── 在线状态心跳（供 DSH 设置页 / 控制台读取） ────────────────────────────────
  // 单看进程在不在、或单看 WS 连着没，都不能回答「他到底通不通」：
  //   桥接进程活着 ≠ QQ 侧连上；QQ 侧连上 ≠ DSH 能回话。
  // 因此把三层状态各自落盘，并带一个心跳时间戳 —— 状态文件本身「多久没更新」
  // 就是判断桥接进程是否已死（被 Android 清理）的唯一可靠依据。
  ...
  const bridgeStatus = {
    ...
    startedAt: Date.now(),
    heartbeatAt: 0,
    qqConnected: false,        // SnowLuma WebSocket 是否 open
    qqConnectedAt: null,       // 最近一次连上的时间
    qqDisconnectedAt: null,    // 最近一次断开的时间
    qqLastError: null,         // 最近一次连接错误（已脱敏为简短消息）
    selfId: null,              // 机器人 QQ 号
    selfNickname: null,        // 机器人昵称
    wsUrl: cfg.snowluma?.wsUrl ?? '',
    httpUrl: cfg.snowluma?.httpUrl ?? '',
    mode: null,
    dshReady: false,
    consolePort: cfg.consolePort ?? 3100,
    lastInboundAt: null,       // 最近一次收到 QQ 消息
    lastOutboundAt: null       // 最近一次成功发出 QQ 消息
  };

  function writeBridgeStatus() {
    bridgeStatus.heartbeatAt = Date.now();
    bridgeStatus.mode = currentMode;
    bridgeStatus.dshReady = dshReady;
    try { atomicWriteJson(STATUS_FILE, bridgeStatus); } catch {}
  }

  function startStatusHeartbeat() {
    writeBridgeStatus();
    const t = setInterval(writeBridgeStatus, STATUS_HEARTBEAT_MS);
    t.unref?.();
  }

  // turn「防卡死」租约：v2TurnStartAt / collectors 正常应在 turn/end 时清除；
  // 若引擎被外部杀掉、事件流静默结束、或 turn/end 帧丢失，这两个标记会让
  // isConversationBusyV2 永久返回 true，导致该会话再也无法被唤醒（死锁）。
  // 这里给每个在途 turn 加 30 分钟兜底，超时强制清除。
  const turnLeaseTimers = new Map(); // sessionId -> timeout
  function armTurnLease(sessionId) {
    const old = turnLeaseTimers.get(sessionId);
    if (old) clearTimeout(old);
    const timer = setTimeout(() => {
      turnLeaseTimers.delete(sessionId);
      const stale = v2TurnStartAt.delete(sessionId) || collectors.delete(sessionId);
      sendToolSucceededSessions.delete(sessionId);
      pendingSendToolCalls.delete(sessionId);
      toolCallNames.delete(sessionId);
      if (stale) log(`⚠️ turn 租约超时，强制清除在途标记 ${sessionId}（会话已可再次唤醒）`);
    }, 30 * 60 * 1000);
    timer.unref?.();
    turnLeaseTimers.set(sessionId, timer);
  }
  function disarmTurnLease(sessionId) {
    const old = turnLeaseTimers.get(sessionId);
    if (old) clearTimeout(old);
    turnLeaseTimers.delete(sessionId);
  }
  function clearAllTurnLeases() {
    for (const t of turnLeaseTimers.values()) clearTimeout(t);
    turnLeaseTimers.clear();
  }
      ...
      try {
        ...
        if (req.method === 'GET' && url.pathname === '/api/status') {
          ...
          sendJson({
            ...
            // 三层连通性：进程活着 / QQ 侧连上 / DSH 能回话，缺一不可
            qqConnected: bot?.isConnected === true,
            qqConnectedAt: bridgeStatus.qqConnectedAt,
            qqDisconnectedAt: bridgeStatus.qqDisconnectedAt,
            qqLastError: bridgeStatus.qqLastError,
            selfId: bridgeStatus.selfId,
            selfNickname: bridgeStatus.selfNickname,
            wsUrl: bridgeStatus.wsUrl,
            httpUrl: bridgeStatus.httpUrl,
            pid: bridgeStatus.pid,
            startedAt: bridgeStatus.startedAt,
            heartbeatAt: bridgeStatus.heartbeatAt,
            lastInboundAt: bridgeStatus.lastInboundAt,
            lastOutboundAt: bridgeStatus.lastOutboundAt,
        ...
        if (req.method === 'GET' && url.pathname === '/api/socialV2/prompt') {
          ...
          sendJson({
            ...
            localPacks: {
              enabled: localStickerPackEnabled(),
              total: localPackEntries.length,
              context: cfg.socialV2?.stickerPack?.includeInPrompt !== false ? buildLocalPackContext(localPackEntries, cfg.socialV2?.stickerPack?.promptMaxPacks ?? 12) : '',
              strategy: cfg.socialV2?.stickerPack?.includeInPrompt !== false ? buildLocalPackStrategyHint() : ''
            }
          });
          return;
        }
        ...
        if (req.method === 'POST' && url.pathname === '/api/socialV2/send-sticker') {
          ...
          try {
            ...
            st.preSleepWaitObservedAt = 0;
            st.preSleepWaitAccumMs = 0;
            st.packCadenceGroup = 0; // 发表情后重置“每N句”节奏计数
            st.packCadencePending = false;
            saveSocialV2State();
        ...
        // ── 本地表情包（表情包集目录下的分类 GIF） ──────────────────────────
        if (req.method === 'GET' && url.pathname === '/api/socialV2/local-sticker-list') {
          if (!localStickerPackEnabled()) { sendJson({ ok: false, error: '本地表情包体系已关闭' }, 403); return; }
          const key = String(url.searchParams.get('key') ?? '').trim();
          const query = String(url.searchParams.get('query') ?? '').trim();
          const count = Math.min(500, Math.max(1, Math.min(Number(url.searchParams.get('count')) || 48, 200)));
          if (!key) { sendJson({ ok: false, error: 'key 不能为空' }, 400); return; }
          if (req.headers['x-agent-token'] && !agentTokenOk(key, req.headers['x-agent-token'])) { sendJson({ ok: false, error: 'agent token 无效' }, 403); return; }
          if (req.headers['x-agent-token'] && !v2SessionAllowed(key)) { sendJson({ ok: false, error: '目标不在当前模式允许范围内' }, 403); return; }
          if (req.headers['x-agent-token'] && !v2ToolEnabled('listLocalStickers')) { sendJson({ ok: false, error: '工具未启用：qq_list_local_stickers' }, 403); return; }
          try {
            const synced = syncLocalPackLibrary(false);
            if (!synced) { sendJson({ ok: false, error: '本地表情包体系已关闭或目录不可用' }, 403); return; }
            const list = formatLocalPackList(synced.entries, query, count);
            sendJson({ ok: true, key, ...list, dir: synced.dir, scannedAt: synced.scannedAt });
          } catch (error) {
            sendJson({ ok: false, error: `获取本地表情列表失败：${error?.message ?? error}` }, 500);
          }
          return;
        }
        if (req.method === 'GET' && url.pathname === '/api/socialV2/local-sticker-image') {
          if (!localStickerPackEnabled()) { sendJson({ ok: false, error: '本地表情包体系已关闭' }, 403); return; }
          const key = String(url.searchParams.get('key') ?? '').trim();
          const stickerId = String(url.searchParams.get('stickerId') ?? '').trim();
          if (!key || !stickerId) { sendJson({ ok: false, error: 'key 和 stickerId 不能为空' }, 400); return; }
          if (req.headers['x-agent-token'] && !agentTokenOk(key, req.headers['x-agent-token'])) { sendJson({ ok: false, error: 'agent token 无效' }, 403); return; }
          if (req.headers['x-agent-token'] && !v2SessionAllowed(key)) { sendJson({ ok: false, error: '目标不在当前模式允许范围内' }, 403); return; }
          if (req.headers['x-agent-token'] && !v2ToolEnabled('getLocalStickerImage')) { sendJson({ ok: false, error: '工具未启用：qq_get_local_sticker_image' }, 403); return; }
          try {
            const entry = await getLocalStickerData(stickerId);
            if (!entry.file || !fs.existsSync(entry.file)) { sendJson({ ok: false, error: `本地表情 ${entry.id} 的文件不存在` }, 404); return; }
            const stat = fs.statSync(entry.file);
            const maxBytes = Math.min(16 * 1024 * 1024, Math.max(0, Number(cfg.socialV2?.stickerPack?.maxBytes) || 8 * 1024 * 1024));
            if (stat.size > maxBytes) { sendJson({ ok: false, error: `本地表情文件过大（${Math.round(stat.size / 1024)}KB > ${Math.round(maxBytes / 1024)}KB）` }, 400); return; }
            const buf = fs.readFileSync(entry.file);
            const dims = getImageDimensions(buf);
            if (dims && dims.width * dims.height > MAX_MEDIA_PIXELS) {
              sendJson({ ok: false, error: `本地表情像素超限（${dims.width}x${dims.height}）` }, 400);
              return;
            }
            ...
            sendJson({
              ok: true,
              key,
              sticker: { id: entry.id, category: entry.category, label: entry.label, hint: entry.hint, mime: entry.mime, size: entry.size },
              image: { mimeType, data: buf.toString('base64') }
            });
          } catch (error) {
            sendJson({ ok: false, error: `获取本地表情图片失败：${error?.message ?? error}` }, 500);
          }
          return;
        }
        if (req.method === 'POST' && url.pathname === '/api/socialV2/send-local-sticker') {
          if (!localStickerPackEnabled()) { sendJson({ ok: false, error: '本地表情包体系已关闭' }, 403); return; }
          const body = await readBody();
          const key = String(body.key ?? '').trim();
          const stickerId = String(body.stickerId ?? '').trim();
          ...
          // 仿真常识：表情消息只能是一张表情，不能在同一气泡里附带文字说明。
          if (caption) {
            sendJson({ ok: false, error: '表情消息不能附带文字；请先用 qq_send_message / qq_reply 把想说的话作为单独气泡发送，再单独发表情' }, 400);
            return;
          }
          if (req.headers['x-agent-token'] && !agentTokenOk(key, req.headers['x-agent-token'])) { sendJson({ ok: false, error: 'agent token 无效' }, 403); return; }
          if (req.headers['x-agent-token'] && !v2SessionAllowed(key)) { sendJson({ ok: false, error: '目标不在当前模式允许范围内' }, 403); return; }
          if (req.headers['x-agent-token'] && !v2ToolEnabled('sendLocalSticker')) { sendJson({ ok: false, error: '工具未启用：qq_send_local_sticker' }, 403); return; }
          if (currentMode !== 'reserved2') { sendJson({ ok: false, error: '该接口仅 reserved2 模式可用' }, 403); return; }
          if (!req.headers['x-agent-token']) { sendJson({ ok: false, error: 'reserved2 模式发送必须携带 agent token' }, 403); return; }
          const keyMatch = /^(group|private):(\d+)$/.exec(key);
          if (!keyMatch) { sendJson({ ok: false, error: 'key 格式应为 group:群号 或 private:QQ号' }, 400); return; }
          ...
          try {
            ...
            st.sendTimes.push(now);
            if (st.sendTimes.length > 500) st.sendTimes = st.sendTimes.slice(-500);
            const sent = await sendLocalStickerV2(key, stickerId, {
              replyToMessageId: actualReplyToMessageId,
              atUserId
            });
            // 记录到二代会话的 recentMessages，让 AI 知道自己发过这个表情
            const label = sent.entry?.label || sent.entry?.category || '本地表情';
            const text = `[表情包:${label}]`;
            st.recentMessages.push({
              messageId: sent.messageId ? String(sent.messageId) : null,
              sender: '我',
              ...
              forwardIds: [],
              hasForward: false,
              sticker: { id: sent.entry?.id || stickerId, category: sent.entry?.category || '', label: sent.entry?.label || '', hint: sent.entry?.hint || '' },
              time: Date.now()
            });
            const recentLimit = Number(cfg.socialV2?.context?.recentLimit) || 100;
            if (st.recentMessages.length > recentLimit) st.recentMessages.splice(0, st.recentMessages.length - recentLimit);
            ...
            saveSocialV2State();
            scheduleReplyCheckV2(key);
            log(`[sticker-pack] 工具发送本地表情 ${key}: ${sent.entry?.id || stickerId}`);
            appendActivity(`${key} [sticker-pack] 工具发送本地表情：${label}`);
            sendJson({ ok: true, key, sticker: sent.entry, sent: 1, failed: 0, quoted: quotedInfo });
          } catch (error) {
            const st = getSocialV2State(key);
            const idx = st.sendTimes.indexOf(now);
            ...
            if (st.sendTimes.length > 500) st.sendTimes = st.sendTimes.slice(-500);
            saveSocialV2State();
            log(`[sticker-pack] 工具发送本地表情失败 ${key}: ${error?.message ?? error}`);
            sendJson({ ok: false, error: `发送本地表情失败：${error?.message ?? error}` }, 500);
          }
          return;
        }
        // 管理端表情库接口（控制台）
    ...
    for (const part of parts) {
      sendChain = sendChain
        .then(async () => {
          ...
          bridgeStatus.lastOutboundAt = Date.now(); // 只在真的发出去之后才记，失败不刷新
        })
  ...
  // 四种 QQ 模式共用 DSH 会话，这里统一把会话模型钉到 config.dsh 指定的 provider/model。
  //
  // 关于 reasoningEffort：并非所有模型都支持思考强度 —— 宿主 llm.models 里只有带
  // reasoning.efforts 的模型才接受该参数（deepseek-official 全系带，opi 的 Claude
  // 全部不带）。带上不支持的 effort 会直接 model-unavailable 失败，导致模型压根
  // 没被设置。因此：
  //   ① config 里 reasoningEffort 留空 → 不传该字段（schema 中它本就是 optional）；
  //   ② 传了但被判定不支持 → 自动去掉 effort 重试一次，保证模型仍能设上。
  // 这样换模型时不必再手工同步 effort 配置。
  const visionModelAppliedSessions = new Set();
  /** 判断错误是否为「该模型不支持思考强度」。 */
  function isEffortUnsupportedError(error) {
    const msg = String(error?.message ?? error ?? '');
    return /does not support reasoning effort|unsupported reasoning effort/i.test(msg);
  }
  async function ensureVisionModel(sessionId) {
    ...
    // 留空（''/null/未配置项显式设为空串）即表示「用模型自己的默认强度」
    const configuredEffort = cfg.dsh?.reasoningEffort;
    let effort = configuredEffort == null ? 'max' : String(configuredEffort).trim();
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        log(`已设置会话模型 ${sessionId} -> ${result.selected.provider}/${result.selected.model} (思考强度: ${result.selected.reasoningEffort ?? '模型默认'})`);
        return;
      } catch (error) {
        // 模型不支持思考强度：去掉该参数立即重试，而不是白白重试同样的请求
        if (effort && isEffortUnsupportedError(error)) {
          log(`模型 ${provider}/${model} 不支持思考强度「${effort}」，改用模型默认强度重试`);
          effort = '';
          continue;
        }
  ...
  async function processPromptQueue(key) {
    ...
    try {
      ...
      if (entry.current === item) entry.current = null;
      entry.running = false;
      // 事件流重连时旧队列可能已被替换；旧 turn 收尾不能删除新队列。
      if (promptQueues.get(key) !== entry) return;
  ...
  function buildWakePromptV2(key, reason) {
    ...
    const stickerLine = stickerCfg.enabled !== false && stickerCfg.includeInPrompt !== false
      ...
      : '';
    const localPackCfg = cfg.socialV2?.stickerPack ?? {};
    const localPackLine = localPackCfg.enabled !== false && localPackCfg.includeInPrompt !== false
      ? `${buildLocalPackStrategyHint()}\n${buildLocalPackContext(localPackEntries, localPackCfg.promptMaxPacks ?? 12)}\n\n`
      : '';
    // 发表情节奏：每连续自发 N 句话，以 probability 概率“强提醒”发一张本地表情包（真随机硬币）。
    // 只影响“该不该提醒”，不替代工具调用；发过表情（本地或QQ收藏）后计数归零重新开始。
    let cadenceLine = '';
    const cadCfg = localPackCfg.cadence ?? {};
    if (localPackCfg.enabled !== false && cadCfg.enabled !== false) {
      const every = Math.max(1, Number(cadCfg.everyMessages) || 3);
      const probRaw = Number(cadCfg.probability);
      const prob = Number.isFinite(probRaw) ? Math.max(0, Math.min(0.95, probRaw)) : 0.5;
      let selfTextCount = 0;
      const rm = Array.isArray(st.recentMessages) ? st.recentMessages : [];
      for (let i = rm.length - 1; i >= 0; i--) {
        const m = rm[i];
        if (!m) continue;
        // 自己发的、非表情/非图片的纯文本消息 → 计数；遇到表情/图片/他人消息即停止。
        if (m.isSelf && !m.sticker && !(Array.isArray(m.media) && m.media.length)) selfTextCount += 1;
        else break;
      }
      const group = Math.floor(selfTextCount / every);
      if (group >= 1 && st.packCadenceGroup !== group) {
        st.packCadenceGroup = group; // 记录已在哪个“每 N 句”分组做过一次二选一
        if (Math.random() < prob) st.packCadencePending = true;
      }
      if (st.packCadencePending) {
        cadenceLine = `【发表情节奏】你已连发 ${selfTextCount} 句话，按规则该发一张表情包了——用 qq_send_local_sticker，一条一张、别带文字。\n\n`;
        st.packCadencePending = false;
      }
    }
    const preSleepMs = Math.max(0, Number(cfg.socialV2?.wake?.preSleepWaitMs) || 300000);
    ...
    const base = roleLine + tokenLine + antiAiLine + proactiveLine + stickerLine + localPackLine + cadenceLine + preSleepLine + statusLine + wakeLine + memoryLine + participationLine;
    if (reason === 'bootstrap') {
  ...
  function hasActiveTurnV2(key) {
    const sid = state.sessions[key];
    return !!(sid && (v2TurnStartAt.has(sid) || collectors.has(sid)));
  }
  ...
  // DSH 重启/事件流断开时，旧版本可能留下“已接受但没有 turn/start”的假 busy。
  // 这种状态不会再自行清除，导致后续 @ 只进入 pendingWakeReasons、永远不唤醒。
  // 对没有任何实际在途 turn 的会话立即清理这类标记；真正进行中的 turn 不受影响。
  function recoverStaleWakeBusy(key, reason) {
    const st = getSocialV2State(key);
    const q = promptQueues.get(key);
    if (hasActiveTurnV2(key) || q?.running || q?.queue?.length) return false;
    let recovered = false;
    if (st.pendingWakeTimer) {
      clearTimeout(st.pendingWakeTimer);
      st.pendingWakeTimer = null;
    ...
    if (pendingWakeKeys.delete(key)) {
      disarmPendingWakeLease(key);
      recovered = true;
    }
    if (recovered) log(`[reserved2] 清理 ${key} 的残留 busy 标记（触发原因：${reason}）`);
    return recovered;
  }
  ...
  function scheduleWakeV2(key, reason) {
    ...
    if (isConversationBusyV2(key, st)) {
      // 只有真实在途 turn/队列才应积压；旧版残留 busy 标记不能阻塞 @。
      if (recoverStaleWakeBusy(key, reason)) return scheduleWakeV2(key, reason);
      if (!Array.isArray(st.pendingWakeReasons)) st.pendingWakeReasons = [];
      const seq = st.lastUnreadSeq || 0;
      if (!st.pendingWakeReasons.some((r) => r && r.reason === reason && r.seq === seq)) {
        st.pendingWakeReasons.push({ reason, seq });
  ...
  async function handleIncoming(kind, id, event, cfg) {
    ...
    bridgeStatus.lastInboundAt = Date.now(); // 收到任意 QQ 消息即刷新（含被拒的未授权会话，用于证明链路真的在收数据）
    if (event?.self_id != null && bridgeStatus.selfId == null) bridgeStatus.selfId = String(event.self_id);
    if (!modeAllowed(key, kind, id, cfg, currentMode)) {
  ...
  async function pumpMux() {
    for (;;) {
      try {
        ...
        for await (const envelope of api.events.mux({})) {
          ...
          if (frame.type === 'session/event') {
            ...
            if (ended) {
              ...
              disarmTurnLease(frame.sessionId);
        ...
        // 关键：事件流可能「静默正常结束」——引擎被外部杀掉/重启时 for await 会干净退出而
        // 不抛异常。清理若只写在 catch 里就不会执行，v2TurnStartAt / collectors 残留在途
        // 标记会让 isConversationBusyV2 永久返回 true，该会话从此再也不会说话（死锁）。
        // 因此统一放进 finally：异常退出与正常结束都清理。
        collectors.clear(); // 清除旧 turn collector，避免重连后残留导致重复累加
        // 事件流断开后，旧 prompt 队列中的 running 项可能永远等不到结果；
        // 清空队列，避免它把后续 @ 误判为“会话繁忙”。旧项的 finally 会因队列已替换而不影响新队列。
        drainAllPromptQueues('DSH 事件流已断开，旧投递已取消');
  ...
  // 连接事件日志：PC 端离线时 SDK 会持续指数退避重连，每轮都触发 close+error。
  // 原样打印会把 bridge.log 刷满（实测离线 4 分钟刷了 27 行 `错误: {}`），
  // 因此对「断开/错误」做节流：首次照常打印，之后同类事件每 5 分钟才打一次并附带折叠次数。
  const CONNLOG_THROTTLE_MS = 5 * 60 * 1000;
  const connLogState = new Map(); // kind -> { last: ts, suppressed: n }
  function logConnEvent(kind, message) {
    const now = Date.now();
    const st = connLogState.get(kind) ?? { last: 0, suppressed: 0 };
    if (now - st.last < CONNLOG_THROTTLE_MS) {
      st.suppressed += 1;
      connLogState.set(kind, st);
      return;
    }
    const tail = st.suppressed > 0 ? `（期间同类事件已折叠 ${st.suppressed} 次）` : '';
    log(message + tail);
    connLogState.set(kind, { last: now, suppressed: 0 });
  }

  bot.on('open', () => {
    connLogState.clear(); // 连上后重置节流，下次真断开能立刻看到
    bridgeStatus.qqConnected = true;
    bridgeStatus.qqConnectedAt = Date.now();
    bridgeStatus.qqLastError = null;
    writeBridgeStatus();
    log(`SnowLuma 已连接：${cfg.snowluma.wsUrl}`);
  });
  bot.on('close', (info) => {
    bridgeStatus.qqConnected = false;
    bridgeStatus.qqDisconnectedAt = Date.now();
    writeBridgeStatus();
    logConnEvent('close', `SnowLuma 连接断开（code=${info?.code ?? '?'}），重连中…`);
  });
  bot.on('error', (error) => {
    const msg = error?.message || error?.name || (typeof error === 'string' ? error : '未知错误');
    bridgeStatus.qqLastError = String(msg).slice(0, 200);
    logConnEvent('error', `SnowLuma 错误: ${msg}`);
  });

  // 状态心跳：5 秒写一次 state/bridge-status.json。
  // 心跳时间戳是判断「桥接进程是否还活着」的唯一可靠依据 —— 进程被 Android
  // 后台清理时不会留任何日志，但心跳会立刻停止更新。
  startStatusHeartbeat();

  // 本地控制台与 DSH 探活先拉起来：它们不依赖 QQ 侧连接。
  // 这样即使 PC 端 SnowLuma 没开机，桥接也能安静地活着（控制台可访问、状态可查），
  // 而不是首次连接失败就 process.exit(1) → start.sh 每 5 秒重启的崩溃循环
  // （崩溃循环还更容易被 Android 后台清理策略当成异常进程杀掉）。
  startDshWatch();
  startConsoleServer();

  // 连接成功后的一次性初始化（读昵称、预热表情库）。
  let botInitialized = false;
  async function initAfterBotConnected() {
    ...
    try {
      ...
      if (login?.nickname) {
        ...
        bridgeStatus.selfNickname = String(login.nickname);
        if (login.user_id != null) bridgeStatus.selfId = String(login.user_id);
        writeBridgeStatus();
  ...
  // 首次连接改为「非致命」：连不上只记日志，不再 process.exit(1)。
  // 重连交给 SDK 自己负责 —— 实测 @snowluma/sdk 的 connect() 在 socket 未 open 就 close 时，
  // 会先 reject（SnowLumaConnectionError）再走 handleClose() → scheduleReconnect()，
  // retries 默认 Infinity、延迟 1s×2^n 封顶 30s。所以这里绝不能自己再写重试循环，
  // 否则会与 SDK 的 reconnectTimer 竞争、开出重复 socket。
  // 初始化改由 'open' 事件驱动（首连或任意一次重连成功都会触发，内部有幂等保护）。
  bot.on('open', () => { void initAfterBotConnected(); });
  try {
    await bot.connect();
  } catch (error) {
    log(
      `SnowLuma 首次连接失败：${error?.message ?? error}；` +
      `已转入后台自动重连（PC 端 SnowLuma 启动后会自动接上）。` +
      `本地控制台仍可用：http://127.0.0.1:${cfg.consolePort}`
    );
  }
