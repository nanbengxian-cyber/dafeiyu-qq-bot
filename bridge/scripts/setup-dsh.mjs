...
// MCP server 的 command 必须是一个真实可执行的 node 二进制。
// Android/Termux 版 DSH 上 process.execPath 会解析成 /apex/com.android.runtime/bin/linker64
// （动态链接器，不能当 node 用），因此不能直接采用；这里按优先级探测：
//   1. 环境变量 DSH_NODE_BIN（显式覆盖）
//   2. PATH 上的 node（command -v / where）
//   3. process.execPath（Windows / 常规 Linux 桌面正常可用）
function resolveNodeBin() {
  ...
  if (exec && /(^|[\\/])node(\.exe)?$/i.test(exec) && fs.existsSync(exec)) return exec;
  fatal('无法确定 node 可执行文件路径；请设置环境变量 DSH_NODE_BIN 指向 node 后重试。');
}
