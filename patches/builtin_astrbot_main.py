        ...
        if group_context_enabled and self.group_chat_context and has_context_content:
            ...
            if need_active:
                ...
                try:
                    ...
                    # [patch:genexit-v1 别把「下游主动掐断」记成错误]
                    #
                    # 现象：一天 1580 条 ERRO，正文是空的 ——
                    #   [ERRO] astrbot.main:272: Traceback (most recent call last):
                    #   [ERRO] astrbot.main:273: 主动回复失败:
                    # 冒号后面什么都没有，traceback 也只剩一行头。查了很久才明白
                    # 这不是 bug 而是**正常控制流**：本函数是异步生成器，上面
                    # yield 出去之后，如果下游插件（dsh-decide 判定这轮不该说话、
                    # dsh-selfguard 判定该拦）调用了 stop_event()，调度器就不再
                    # 向本生成器要下一个值，而是直接 aclose() 它 —— Python 于是
                    # 在 yield 处抛 GeneratorExit。它的 str() 是空串，被下面那个
                    # `except BaseException` 捞住，就打成了那行空错误。
                    #
                    # 三个后果都实在：
                    #   1. 每天 1580 条假 ERRO 淹掉真错误，排查全是噪音；
                    #   2. 在 GeneratorExit 里还调 logger，属于「关闭期间继续
                    #      干活」，本身就不干净；
                    #   3. 看日志的人会反复误判「主动回复挂了」。
                    # 必须重新抛出，否则 Python 报 generator ignored GeneratorExit。
                    raise
                except BaseException as e:
